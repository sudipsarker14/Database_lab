...
SELECT * FROM Employee WHERE Salary> (SELECT avg(Salary) from Employee)
and Salary > (Select avg(Budget) from Department);
SELECT avg(Salary) FROM Employee 
WHERE DepartmentID = any(select DepartmentId from Department where DepartmentName = 'HR');
SELECT avg(Salary) FROM Employee 
WHERE DepartmentID = any(select DepartmentId from Department where DepartmentName = 'HR' or DepartmentName = 'IT');
